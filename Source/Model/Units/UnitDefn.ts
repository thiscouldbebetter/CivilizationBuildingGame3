...
class UnitDefn
{
	...
	movement: UnitDefnMovement;
	combat: UnitDefnCombat;
	actionsAvailableNames: string[];
	symbol: string;
	...
	static byName(name: string): UnitDefn
	{
		return UnitDefn.Instances().byName(name);
	}

	static _instances: UnitDefn_Instances;
	static Instances(): UnitDefn_Instances
	{
	...
	actionsAvailable(): UnitActivityDefn[]
	{
		return this.actionsAvailableNames.map
		(
			(x: string) => UnitActivityDefn.byName(x)
		);
	}
...
{
	AegisCruiser: UnitDefn;
	AlpineTroops: UnitDefn;
	Archers: UnitDefn;
	Armor: UnitDefn;
	Artillery: UnitDefn;
	Battleship: UnitDefn;
	...
	Legion: UnitDefn;
	Marines: UnitDefn;
	MechanizedInfantry: UnitDefn;
	Musketeers: UnitDefn;
	NuclearMissile: UnitDefn;
	Paratroopers: UnitDefn;
	Partisans: UnitDefn;
	Phalanx: UnitDefn;
	Pikemen: UnitDefn;
	Riflemen: UnitDefn;
	Settlers: UnitDefn;
	Spy: UnitDefn;
	StealthBomber: UnitDefn;
	StealthFighter: UnitDefn;
	Submarine: UnitDefn;
	Transport: UnitDefn;
	Trireme: UnitDefn;
	Warriors: UnitDefn;
	...
	_All: UnitDefn[];
	_AllByName: Map<string, UnitDefn>;

	constructor()
	{
		// Movements.
		var ma = (movesPerTurn: number) => UnitDefnMovement.air(movesPerTurn);
		var mg = (movesPerTurn: number) => UnitDefnMovement.ground(movesPerTurn);
		var mo = (movesPerTurn: number) => UnitDefnMovement.ocean(movesPerTurn);
		...
		var a0 = new Array<UnitActivityDefn>();
		...
		var aFre = new Array<UnitActivityDefn>();
		...
		var ud =
		(
			a: string, b: number, c: UnitDefnMovement, d: UnitDefnCombat,
			e: UnitActivityDefn[], f: string, g: number
		) => new UnitDefn(a, b, c, d, e.map(x => x.name), f, g);
		var c =
		(
			a: number, b: number, c: number, d: number
		) => new UnitDefnCombat(a, b, c, d); // attack, defense, integrityMaxOver10, damagePerHit
		// var udTodo =
			// () =>
			// {
				// return new UnitDefn
