...
class Base
{
	...
	static fromNamePosAndOwnerName
	(
		name: string,
		pos: Coords,
		ownerName: string
	...
	distanceFromCapital(world: World): number
	{
	...
	isCoastal(world: World): boolean
	{
		var map = world.map;
		var cellsNeighboring =
			map.cellsNeighboringCellAtPosInCells(this.pos);
		var areAnyNeighboringCellsOcean = cellsNeighboring.some
		(
			(x: MapOfCellsCell) => x.terrain(world).isOcean()
		);
		return areAnyNeighboringCellsOcean;
	}

	mapCellOccupied(world: World): any
	{
		return world.map.cellAtPosInCells(this.pos);
	}

	mapCellsUsable(world: World): any[]
	{
	...
	{
		var returnValue = world.owners.find
		(
			(x: Owner) => (x.name == this.ownerName)
		);
		return returnValue;
	}
	...
	hasImprovement(improvement: BaseImprovementDefn): boolean
	{
		...
		(
			(x: string) => x == improvement.name
		);
	}

	hasImprovementPoliceStation(): boolean
	{
	...
	moneyThisTurnNetIsNegative(world: World): boolean
	{
	...
	resourcesProducedThisTurn(world: World): ResourceProduction
	{
