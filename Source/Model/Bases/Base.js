"use strict";
class Base {
    constructor(name, pos, ownerName, demographics, landUsage, foodStockpiled, industry, improvementsPresentNames) {
    ...
    distanceFromCapital(world) {
        ...
        var returnValue = Math.floor(baseCapital.pos.clone().subtract(this.pos).absolute().magnitude());
        return returnValue;
    }
    initialize(world) {
    ...
    isCoastal(world) {
        var map = world.map;
        var cellsNeighboring = map.cellsNeighboringCellAtPosInCells(this.pos);
        var areAnyNeighboringCellsOcean = cellsNeighboring.some((x) => x.terrain(world).isOcean());
        return areAnyNeighboringCellsOcean;
    }
    ...
    owner(world) {
        var returnValue = world.owners.find((x) => (x.name == this.ownerName));
        return returnValue;
    }
        ...
        var improvements = "Improvements: "
            +
                (improvementsPresent.length == 0
                    ? "[none]"
                    :
                        (improvementsPresent.map(x => x.name + " (" + x.costPerTurn + ")").join(", ")));
        var landUsage = this.landUsage.toStringVisualForWorldAndBase(world, this);
        var lines = [
            "Name: " + this.id,
    ...
    hasImprovementPoliceStation() {
        return this.hasImprovement(BaseImprovementDefn.Instances().PoliceStation);
    }
    ...
    buildableStart(buildableToBuild, world) {
        var owner = this.owner(world);
        this.industry.buildableStart(buildableToBuild, world, owner, this);
    }
    buildablesAvailable(world) {
        var owner = this.owner(world);
        var buildablesKnown = owner.buildablesKnown();
        var baseImprovementsPresent = this.improvementsPresent();
        var buildablesAvailable = buildablesKnown.filter((buildable) => {
            var typeName = buildable.constructor.name;
            var isAvailable = (typeName == UnitDefn.name
                ||
                    (typeName == BaseImprovementDefn.name
                        &&
                            ((baseImprovementsPresent.some(i => i.name == buildable.name) == false)
                                &&
    ...
    corruptionPerUnitDistanceFromCapital(world) {
        ...
        return government.corruptionPerUnitDistanceFromCapital;
    }
    corruptionThisTurn(world) {
        var distanceFromCapital = this.distanceFromCapital(world);
        var owner = this.owner(world);
        var corruptionPerUnitDistance = owner.corruptionPerUnitDistanceFromCapital();
        var corruptionThisTurn = Math.floor(corruptionPerUnitDistance * distanceFromCapital);
    ...
    foodThisTurnNet(world) {
        ...
        var consumedByPopulation = this.population() * this.foodConsumedPerPopulation();
        var unitsSupported = this.unitsSupported(world);
        var settlersSupported = unitsSupported.filter(x => x.defnName == UnitDefn.Instances().Settlers.name);
        var settlerCount = settlersSupported.length;
        var consumedBySettlers = settlerCount * this.foodConsumedPerSettler(world);
        var consumed = consumedByPopulation + consumedBySettlers;
    ...
    industryNeededToSupportUnits(world) {
        var owner = this.owner(world);
        var government = owner.government();
        var industryNeeded = government.industryConsumedByUnitsSupportedByBase(this);
    ...
    luxuriesThisTurn(world) {
        var luxuriesFromTrade = this.luxuriesThisTurnFromTrade(world);
        var luxuriesFromEntertainers = this.demographics.entertainerCount * this.luxuriesPerEntertainer();
        var luxuriesThisTurn = luxuriesFromTrade
    ...
    moneyThisTurnGross(world) {
        ...
        var moneyFromTaxCollectors = this.demographics.taxCollectorCount
    ...
    moneyThisTurnNetIsNegative(world) {
        return (this.moneyThisTurnNet(world) < 0);
    }
    ...
    researchThisTurn(world) {
        ...
        var researchFromScientists = this.demographics.scientistCount
    ...
    researchThisTurnFromTrade(world) {
        ...
        var researchThisTurn = Math.round(researchPlusLuxuriesThisTurn
            * researchRate
    ...
    // Units.
    unitRemove(unit) {
        this.unitsSupportedIds.splice(this.unitsSupportedIds.indexOf(unit.id), 1 // numberToRemove
        );
    }
    unitSupport(unit) {
    ...
    unitsPresentMilitary(world) {
        ...
        var returnValues = unitsPresent.filter((unit) => {
            var defn = unit.defn(world);
            var isGroundMilitary = (defn.isMilitary() && defn.isGround(world, unit));
