...
class BaseDemographics {
    constructor(population, entertainerCount, scientistCount, taxCollectorCount) {
    ...
    attitudeIsEuphoria(world, base) {
        var discontentPopulationCount = this.populationDiscontent(world, base);
        var happyPopulationCount = this.populationHappy(world, base);
        var contentPopulationCount = this.population
            - happyPopulationCount
            - discontentPopulationCount;
        var returnValue = (this.population >= 3
    ...
    populationContent(world, base) {
        var returnValue = this.population
    ...
    populationDiscontent(world, base) {
        ...
        var populationMaxBeforeDiscontent = difficultyLevel.basePopulationBeforeDiscontent;
        var unhappinessDueToOverpopulation = this.population - populationMaxBeforeDiscontent;
        ...
        var doesDeploymentCauseDiscontent = (owner.governmentIs(governments.Republic)
        ...
        if (doesDeploymentCauseDiscontent) {
            ...
            var unitsMilitaryDeployed = unitsMilitary.filter(x => x.isInBaseSupporting(world) == false);
        ...
        var discontentPopulationCount = unhappinessDueToOverpopulation
            + unhappinessDueToMilitaryDeployment;
        var improvementsPresent = base.improvementsPresent();
        if (discontentPopulationCount > 0) {
            var unhappinessMitigatedByImprovements = 0;
            improvementsPresent.forEach(x => unhappinessMitigatedByImprovements +=
            ...
            var unhappinessMitigatedByMartialLaw = unitsPresentMilitary.length;
            var martialLawMax = ownerGovernment.discontentPopulationMitigatedByMartialLaw;
            if (unhappinessMitigatedByMartialLaw > martialLawMax) {
                unhappinessMitigatedByMartialLaw = martialLawMax;
            }
            var unhappinessMitigatedTotal = unhappinessMitigatedByImprovements
    ...
    hasSpecialists() {
        var returnValue = (this.entertainerCount > 0
    ...
    specialistCount() {
        var returnValue = this.entertainerCount
            + this.scientistCount
            + this.taxCollectorCount;
        return returnValue;
    }
