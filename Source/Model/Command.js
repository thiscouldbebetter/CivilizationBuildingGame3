...
class CommandOpcode_Instances {
    constructor() {
        var co = (text, keyboardKeys, execute) => new CommandOpcode(text, keyboardKeys, execute);
        var keysNone = new Array();
    ...
    byCommandText(commandText) {
        var opcodeFound = this._All.find((x) => commandText.startsWith(x.text));
        return opcodeFound;
    }
    byKeyboardKey(key) {
        var opcodeFound = this._All.find((x) => x.keyboardKeys.indexOf(key) >= 0);
        return opcodeFound;
    }
    // Commands.
    cityList(u, w, c) {
    ...
    help(u, w, c) {
        ...
        CommandOpcode.Instances()._All.forEach(x => u.outputLog.writeLine(x.text));
    }
    selectionShow(u, w, c) {
        var outputLog = u.outputLog;
        outputLog.clear();
    ...
    techList(u, w, c) {
        ...
        var technologiesAvailable = new Array(); // todo
        technologiesAvailable.forEach((x) => u.outputLog.writeLine(x.text));
    }
    techShow(u, w, c) {
        var outputLog = u.outputLog;
        outputLog.clear();
    ...
    unitActionStart(u, w, c) {
        ...
        if (unit == null) {
            ...
            if (isNaN(actionIndex)
                ...
                outputLog.writeLine("Invalid action number: "
                    + (actionIndex + 1) + ".");
            }
            else {
                var actionToPerform = actionsAvailable[actionIndex]; // todo
                outputLog.writeLine("Unit " + unit.id + " doing: "
                    + actionToPerform.name + ".");
                unit.activityDefnStart(actionToPerform);
                w.draw(u);
            }
        }
    }
    unitActionsShow(u, w, c) {
        ...
        if (unit == null) {
            ...
            actionsAvailableNames.forEach((x, i) => outputLog.writeLine((i + 1) + ": " + x));
        }
    }
    unitList(u, w, c) {
        var owner = w.ownerCurrent();
    ...
    unitMove_Move(u, w, c) {
        ...
        if (directionToMove == null) {
            ...
            var canMove = unitSelected.canMoveInDirection(directionToMove, w);
            if (canMove) {
