...
class MapOfCellsCell {
    constructor(pos, terrainCode, resourceSpecialPresentCode, hasPollution, basePresentId, improvementsPresentNames, unitsPresentIds) {
        this.pos = pos;
    ...
    static fromPosAndCodes(pos, codes) {
        ...
        return new MapOfCellsCell(pos, terrainCode, resourceSpecialCode, null, null, null, null);
    }
    ...
    canBeIrrigated() {
        var returnValue = (this.hasIrrigation() == false
    ...
    resourcesProduced(world, base) {
        ...
        var resources = this._resourcesProducedThisTurn.overwriteWith(terrain.resourceProductionPerTurn);
    ...
    resourceSpecialPresent() {
        var returnValue = (this.resourceSpecialPresentCode == null
            ? null
    ...
    unitsPresentOwnedBySameOwnerAsUnit(unit, world) {
        var unitsPresent = this.unitsPresent(world);
        var unitsOwnedBySameOwnerPresent = unitsPresent.filter(x => (x.id != unit.id
            && x.ownerName == unit.ownerName));
        return unitsOwnedBySameOwnerPresent;
    }
    unitsPresentQualifiedToBePassengersOnUnit(unit, world) {
        var unitsOwnedBySameOwnerPresent = this.unitsPresentOwnedBySameOwnerAsUnit(unit, world);
        var unitsQualifiedToBePassengers = unitsOwnedBySameOwnerPresent.filter(unit => unit.isGround(world) && unit.isSleeping());
