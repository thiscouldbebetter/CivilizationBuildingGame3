"use strict";
class Technology {
    constructor(name, researchToLearn, prerequisiteNames, governmentAllowedName, buildablesAllowedNames) {
        this.name = name;
    ...
    prerequisitesAreSatisfiedByTechnologies(technologiesKnown) {
        var areAnyPrereqsMissing = this.prerequisiteNames.some(prereqName => technologiesKnown.some(tech => tech.name == prereqName) == false);
...
class Technology_Instances {
    constructor() {
        var t = (n, rtl, pn, ga, ba) => new Technology(n, rtl, pn, (ga == null ? null : ga.name), ba.map(x => x.name));
        var costTodo = 10;
        var prereqsNone = new Array();
        var buildablesNone = new Array();
        var govNone = null;
