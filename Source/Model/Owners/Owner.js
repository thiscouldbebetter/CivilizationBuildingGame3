"use strict";
class Owner {
    constructor(name, colorName, intelligence, finances, research, mapKnowledge, diplomacy, notificationLog, bases, units) {
        this.name = name;
    ...
    hasWon(world) {
        var hasConqueredWorld = (world.owners.length == 1
            && world.owners[0] == this);
        var hasReachedNeighboringStarsystem = this.starshipStatus.hasReachedDestination(world);
        var hasWon = (hasConqueredWorld
    ...
    score(world) {
        ...
        var pointsForPopulationHappy = populationHappySoFar * pointsPerPopulationHappy;
        var pointsForPopulationContent = populationContentSoFar * pointsPerPopulationContent;
        var pointsForWonders = wonderCount * pointsPerWonder;
        var pointsLostToPollution = cellsControlledWithPollutionCount * pointsLostPerCellWithPollution;
    ...
    governmentChoiceNeedsInput() {
        var returnValue = (this.governmentIsAnarchy()
    ...
    researchThisTurn(world) {
        var researchSoFar = 0;
        this.bases.forEach(x => researchSoFar += x.researchThisTurn(world));
