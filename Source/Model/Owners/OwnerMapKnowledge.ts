...
class OwnerMapKnowledge
{
	...
	static default(): OwnerMapKnowledge
	{
		return new OwnerMapKnowledge(null);
	}

	draw(universe: Universe, world: World, owner: Owner): void
	{
	...
	(
		...
		mapSizeInCells: Coords,
		mapComplete: MapOfCells,
	...
	update_Group_CellOffset
	(
		world: World,
		owner: Owner,
		viewerPos: Coords,
		cellOffsetPos: Coords
	): void
	{
		var map = world.map;
		var mapSizeInCells = map.sizeInCells;
		...
		var cellAdjacentPos =
			this._cellAdjacentPos.overwriteWith
			(
				viewerPos
			).add
			(
		...
		if (cellAdjacentPos.isInRangeMaxExclusive(mapSizeInCells))
		{
			...
			var areUnitsOrBasesBelongingToOthersPresent =
				unitsOrBasesPresent.some(x => x.ownerName != owner.name);
			if (areUnitsOrBasesBelongingToOthersPresent)
			{
				var unitsOrBasesBelongingToOthers =
					unitsOrBasesPresent.filter(x => x.ownerName != owner.name);
				...
				for (var u = 0; u < unitsOrBasesBelongingToOthers.length; u++)
				{
					...
					var isOwnerOtherKnown =
						ownerDiplomacy.ownerIsKnown(ownerOther);
