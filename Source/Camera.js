...
class Camera {
    ...
    viewCornersClockwiseFromLowerRight() {
        var ori = this.loc.ori;
        var viewCenter = ori.forward.clone().multiplyScalar(focalLength).add(this.loc.pos);
        var offsetFromCenterToEdgeRight = ori.right.clone().multiplyScalar(this.viewSizeHalf.x);
        var offsetFromCenterToEdgeLower = ori.down.clone().multiplyScalar(this.viewSizeHalf.y);
