...
class InputHelper {
    ...
    keyDown(universe, event) {
        ...
        if (key == "Enter") {
            ...
            if (command == null) {
                outputLog.clearAndWriteLine("Unrecognized command: " + commandText);
            }
            else {
            ...
            if (functionKeyNumber < 1
                ...
                outputLog.writeLine("Unrecognized function key number: " + functionKeyNumber + ".");
            }
            else {
                var actionToPerformNumber = "" + functionKeyNumber;
                var command = new Command(CommandOpcode.Instances().UnitActionStart, [
